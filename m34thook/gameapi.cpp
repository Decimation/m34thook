...
{
	return reinterpret_cast<idStr*>(reinterpret_cast<char*>(obj) + idType::FindClassField("idEntity", "name")->offset)->
		data;
}
...
char* get_engine()
{
	if (g_engine_t == nullptr) {
		g_engine_t = (reinterpret_cast<char*>(descan::g_global_typeinfo_tools) -
			idType::FindClassField("engine_t", "typeInfoTools")->offset);
	}
...
struct idDebugHUD_vtbl
{
	...
	void (__fastcall*Printf11)(idDebugHUD* thiz, int, int, const idColor*, const idColor*, const float, const int,
	                           const char*, char*);
	void (__fastcall*Printf)(idDebugHUD* thiz, int, int, const idColor*, const idColor*, const float, const int,
	                         const char*, char*);
	void*            unk2;
...
{
	if (!g_debughud) {
		g_debughud = *reinterpret_cast<idDebugHUD**>(get_engine() + idType::FindClassField("engine_t", "debugHUD")->
			offset);
	}
	return g_debughud;
}
...
void* locate_resourcelist_member_from_idResourceList(void*       reslist /* example:"idDeclEntityDef" */,
                                                     const char* member_name, bool end_at_dollar)
{
...
MH_LEAF
idEventArg mh_ScriptCmdEnt_idEntity(idEventDef* MH_NOESCAPE tdef_name, void* MH_NOESCAPE self,
                                    idEventArg* MH_NOESCAPE args)
{
...
MH_LEAF
void mh_ScriptCmdEnt_idEntity(idEventDef* MH_NOESCAPE tdef_name, void* MH_NOESCAPE self, idEventArg* MH_NOESCAPE args,
                              idEventArg* MH_NOESCAPE out_arg)
{
	void* callev = VTBL_MEMBER(idEntity, VTBLOFFS_CALLEVENT)::Get();
...
*/
bool toggle_idplayer_boolean(void* player, const char* property_name, bool use_explicit_value, bool explicit_value,
                             bool  silent)
{
...
MH_LEAF
void mh_ScriptCmdEnt_idEntity_void(idEventDef* MH_NOESCAPE tdef_name, void* MH_NOESCAPE self,
                                   idEventArg* MH_NOESCAPE args)
{
...
bool reload_decl(void* decl_ptr)
{
	...
	if (resourcelist_for == g_idmaterial2_resourceList_t) {
		*g_idMaterial2_CompositeMaterialStreamingPacket(decl_ptr, "idMaterial2", "CompositeMaterialStreamingPacket") =
			0;
	...
	auto callfn = reinterpret_cast<bool (*)(void*, idStr*)>(reinterpret_cast<void***>(decl_ptr)[0][
		located_decl_read_prod_file]);
